...
class RetrieveDocumentsNode(BaseNode):
    """检索文档节点"""
...
class AdvancedRetrievalNode(BaseNode):
    ...
    async def __call__(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            # 1. 混合检索（通过上下文获取session_id）
            session_id = self.session_id
            candidates = self.hybrid_retriever.retrieve(user_input, session_id, top_k=15)
...
def create_nodes(context_manager=None):
    ...
    return {
        ...
        "retrieve_documents": RetrieveDocumentsNode(context_manager=context_manager),
